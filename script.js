...
const stories = [
  { username: 'reyhaneh.alt', trayName: 'reyhaneh.alt', avatar: 'image/reyAvatar.jpg', seen: false,
    items: [{ type: 'video', src: 'image/reyStory.mp4' }] },
  { username: 'farzin_n1991', trayName: 'farzin_dev', avatar: 'image/farAvatar.jpg', seen: false,
    items: [{ type: 'image', src: 'image/farStory.jpg' }] },
  { username: 'nastaran__dev', trayName: 'nastaran__dev', avatar: 'image/photo_2025-12-07_15-09-20.jpg', seen: false,
    items: [{ type: 'image', src: 'image/storyNas1.jpg' }, { type: 'image', src: 'image/storyNas2.jpg' }] },
  { username: 'parsa_dehghanpour_dev', trayName: 'parsa_deh...', avatar: 'image/deh.jpg', seen: false,
    items: [{ type: 'image', src: 'image/parStory.jpg' }] },
  { username: 'yasaman_dev', trayName: 'yasaman_dev', avatar: 'image/yas.jpg', seen: false,
    items: [{ type: 'image', src: 'image/yasStory.jpg' }] },
  { username: 'setayeshmehr.dev', trayName: 'setayeshm...', avatar: 'image/amiAvatar.jpg', seen: false,
    items: [{ type: 'image', src: 'image/anirStory.jpg' }] },
  { username: 'hivadi.dev', trayName: 'hivadi_dev', avatar: 'image/hivaAvatar.jpg', seen: false,
    items: [{ type: 'image', src: 'image/hivStory.jpg' }] },
  { username: 'pegahmobasherii', trayName: 'pegahmoba...', avatar: 'image/pegAvatar.jpg', seen: false,
...
const post = {
  ...
  caption: 'دیدم بعضی وقتها سوالاتی در رابطه با الگوریتم های گاربج کالکتور میشه،مخصوصا الگوریتم\nmark and sweep\n🌹 خدمت شما',
  likes: 1034,
  liked: false,
  saved: false,
  comments: [
    { username: 'hivadi.dev', avatar: 'image/hivaAvatar.jpg', text: 'تو reference counting ارجاع‌های حلقوی پاک نمیشن، mark and sweep این مشکل رو نداره', likes: 9, liked: false },
    { username: 'nastaran__dev', avatar: 'image/photo_2025-12-07_15-09-20.jpg', text: 'خیلی خوب توضیح دادی 👌', likes: 6, liked: false },
    { username: 'reyhaneh.alt', avatar: 'image/reyAvatar.jpg', text: 'قسمت بعدی درباره‌ی generational GC باشه لطفاً', likes: 4, liked: false },
    { username: 'farzin_n1991', avatar: 'image/farAvatar.jpg', text: 'مرسی، دقیقاً دنبال همین بودم 🙏', likes: 3, liked: false },
    { username: 'yasaman_dev', avatar: 'image/yas.jpg', text: '🔥🔥🔥', likes: 2, liked: false }
  ]
};

// Heart icon path (used in several places)
const heartPath = 'M12 20.25c-.35 0-8.75-4.85-8.75-10.6A4.9 4.9 0 0 1 8.1 4.75c1.7 0 3 .9 3.9 2.35.9-1.45 2.2-2.35 3.9-2.35a4.9 4.9 0 0 1 4.85 4.9c0 5.75-8.4 10.6-8.75 10.6Z';
    ...
    html += `
      <li class="storyAvatar flex w-[70px] shrink-0 cursor-pointer flex-col items-center gap-1" data-index="${index}">
        <div class="story-ring ${user.seen ? 'seen' : ''}">
          <img src="${user.avatar}" alt="" draggable="false" class="h-[62px] w-[62px] rounded-full border-2 border-white object-cover" />
        </div>
        <span class="w-full truncate text-center text-xs">${user.trayName}</span>
      </li>`;
  });
...
postBox.innerHTML = `
  <article class="border-b border-gray-200 pb-4">
    ...
    <div class="flex items-center gap-3 px-3 py-2.5">
      <img src="${post.avatar}" alt="" class="h-8 w-8 rounded-full object-cover" />
    ...
    <div class="relative h-[500px] bg-black">
      <video id="postVideo" src="${post.video}" class="h-full w-full cursor-pointer object-cover" autoplay muted loop playsinline></video>
    ...
    <div class="px-3 pt-1 text-sm">
      ...
      <p class="whitespace-pre-line text-gray-800" dir="auto">${post.caption}</p>
    </div>
...
document.getElementById('soundToggle').addEventListener('click', () => {
  ...
  if (postVideo.muted) {
    ...
    soundWaves.setAttribute('d', 'M15.5 8.5a5 5 0 0 1 0 7M19 5a10 10 0 0 1 0 14');
  }
});
...
function updateCommentCount() {
  ...
  viewComments.textContent = count === 0 ? 'Add a comment...' : `View all ${count} comments`;
}
...
document.getElementById('sheetBackdrop').addEventListener('click', closeComments);
...
function renderComments() {
  ...
  post.comments.forEach((comment, index) => {
    ...
    li.innerHTML = `
      <img src="${comment.avatar}" alt="" class="h-8 w-8 shrink-0 rounded-full object-cover" />
      <div class="min-w-0 flex-1">
        ...
        <div class="mt-1.5 flex gap-4 text-xs font-semibold text-gray-500">
          ...
          ${isMine ? '<button class="editBtn">Edit</button><button class="deleteBtn">Delete</button>' : ''}
        </div>
        <ul class="replies mt-3 flex flex-col gap-3"></ul>
      </div>
      <svg class="likeComment h-4 w-4 shrink-0 cursor-pointer self-center" fill="${comment.liked ? '#ff3040' : 'none'}"
        stroke="${comment.liked ? '#ff3040' : '#737373'}" stroke-width="2" stroke-linejoin="round" viewBox="0 0 24 24">
        <path d="${heartPath}"/>
      </svg>`;
    ...
    (comment.replies || []).forEach((reply) => {
      ...
      replyLi.querySelector('.replyText').textContent = reply.text;
...
function showHome() {
  ...
  document.getElementById('profileBtn').classList.remove('ring-2', 'ring-black');
  playPostVideo();
}
